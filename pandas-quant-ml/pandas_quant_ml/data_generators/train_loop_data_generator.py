...
class TrainTestLoop(object):
    ...
    def train_test_iterator(
        ...
        for train_test in self._train_test_batches(frames, nth_row_only):
            for cache, (features, labels, weights) in zip([train_cache, test_cache], train_test):
    ...
    def _train_test_batches(
            ...
            frames: pd.DataFrame | Iterable[Tuple[Any, pd.DataFrame]] | Dict[Any, pd.DataFrame],
            nth_row_only: int = None,
    ) -> Generator[Tuple[Batch, Batch], Tuple[Batch, Batch], None]:
        for name, df in make_top_level_row_iterator(make_iterable(frames)):
            ...
            idx = unique_level_values(feature_df)
            if len(idx) - test_length <= 0:
                feature_train_df, label_train_df, weight_train_df = feature_df, label_df, weight_df
                feature_test_df, label_test_df, weight_test_df = feature_df.iloc[:0], label_df.iloc[:0], weight_df.iloc[:0]
            else:
                train_test_split_index = idx[len(idx) - test_length]
                if self.train_test_split_ratio[1] > 0:
                    # TODO if validation data needed reserve some % of the test data as test data and use the rest as validation data
                    pass

                feature_train_df, label_train_df = feature_df.loc[:train_test_split_index], label_df.loc[:train_test_split_index]
                weight_train_df = weight_df.loc[:train_test_split_index]
                feature_test_df, label_test_df = feature_df.loc[train_test_split_index:], label_df.loc[train_test_split_index:]
                weight_test_df = weight_df.loc[train_test_split_index:]

            # make batches
            bs_train = len(feature_train_df) if self.batch_size is None else self.batch_size
            bs_test = len(feature_test_df) if self.batch_size is None else self.batch_size
            yield (
                (Batch(feature_train_df, bs_train), Batch(label_train_df, bs_train), Batch(weight_train_df, bs_train), ),
                (Batch(feature_test_df, bs_test), Batch(label_test_df, bs_test), Batch(weight_test_df, bs_test), )
    ...
    def inference_generator(
        ...
        def wrapped_predictor(batches):
            predicted_dfs = []
            for b in batches:
                values = b.values.reshape(-1, *self.label_shape) if self.label_shape is not None else b.values
                predicted = predictor(values)
                predicted_dfs.append(pd.DataFrame(predicted, index=b.index))

            return pd.concat(predicted_dfs, axis=0)

        for name, df in self._inference_generator(frames, wrapped_predictor):
            yield name, df
    ...
    def _inference_generator(
        ...
        for name, df in make_top_level_row_iterator(make_iterable(frames)):
            queue = [df]
            label_df = self._label_pipelines[name].transform(df, queue)
            batcher = Batch(label_df, self.batch_size or len(label_df))
            predicted_df = predictor(batcher)
            yield name, self._label_pipelines[name].inverse(predicted_df)
